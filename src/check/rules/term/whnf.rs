...
use crate::syntax::core::{ConHead, Decl, Elim, Term, Val};

pub fn simplify(tcs: TCS, term: Term) -> ValTCM {
    match term {
        Term::Whnf(whnf) => Ok((whnf, tcs)),
        Term::Redex(def, id, elims) => match tcs.def(def) {
            Decl::Data(_) => Ok((Term::inductive_val(def, elims_to_terms(elims)?), tcs)),
            Decl::Codata(_) => Ok((Term::coinductive_val(def, elims_to_terms(elims)?), tcs)),
            Decl::Cons(cons) => {
